...
class LoRA(nn.Module):
    def __init__(self, original_layer, r=4, alpha=32):
        ...
        self.A = nn.Parameter(torch.empty((weight.shape[0], r)))
        self.B = nn.Parameter(torch.zeros((r, weight.shape[1])))
    ...
    def forward(self, x):
        ...
        layer_output = self.original_layer(x) + (x @ (self.A @ self.B))*self.scaling
